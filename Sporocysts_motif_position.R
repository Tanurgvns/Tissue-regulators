...
sporocyts_allMatchedMotifs <- read.table("C:/Users/Suryansh Raghuvanshi/Downloads/xstreme-stemPromo-shuffle-matches/allMatchedMotifs.tsv", sep="\t", header=TRUE)

# Manually input the MEME-JASPAR ID pairs
stem2_mejsp <- data.frame(
  motif_alt_id = c("STREME-1", "STREME-2", "STREME-3", "STREME-4", "STREME-5"),
  jaspar_id = c(NA, "MA1451.1", "MA0924.1", NA, NA),
  jaspar_name = c("", "nhr-6", "pal-1", "", "")
)

# Read the gene marker order file
stem_markerOrder <- read.table("C:/Users/Suryansh Raghuvanshi/Desktop/Sporocyts_stem_marker.txt", sep="", header=TRUE)
